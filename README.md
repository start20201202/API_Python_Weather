...
def get_weather_history():
    ...
    for city_name, coords in cities.items():
        ...
        if response.status_code == 200:
            data = response.json()['daily']
            for i in range(len(data['time'])):
                # Конвертуємо рядок дати у об'єкт datetime для Google Sheets
                date_obj = datetime.strptime(data['time'][i], '%Y-%m-%d')
                # Формат який Google Sheets розпізнає як дату
    ...
    if all_rows:
        ...
        try:
            sheet.format("B2:B1000", {"numberFormat": {"type": "DATE", "pattern": "yyyy/mm/dd"}})
            print("Формат дати встановлено.")
        except Exception as e:
            print(f"Формат дати не встановлено: {e}")
